...
def get_args_parser():
    ...
    parser.add_argument('--lr_backbone_names', default=['backbone.0', 'crop_backbone'], type=str, nargs='+')
    ...
    parser.add_argument('--lr_drop', default=[45,100], type=int, nargs='+')
    parser.add_argument('--clip_max_norm', default=0.08, type=float,#原来是0.1
    ...
    # SFD module
    parser.add_argument('--use_sfd', action='store_true', help='Enable SFD (DConv) module between backbone and projection')
    parser.add_argument('--sfd_atoms', default=512, type=int, help='Internal atom channels for DConv')
    parser.add_argument('--sfd_alpha', default=0.8, type=float, help='Mixing alpha for DConv (alpha * new + (1-alpha) * orig)')

    # Resume behavior
    parser.add_argument('--preserve_resume_lr', action='store_true', help='When resuming from checkpoint, preserve the optimizer and lr_scheduler state from the checkpoint instead of overriding with current args')
