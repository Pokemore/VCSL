...
def targets_to(targets: List[Dict[str, Any]], device):
    ...
    def move_to_device(x):
        # recursively move tensors to device; leave other python objects unchanged
        if isinstance(x, torch.Tensor):
            return x.to(device)
        elif isinstance(x, dict):
            return {kk: move_to_device(vv) for kk, vv in x.items()}
        elif isinstance(x, list):
            return [move_to_device(vv) for vv in x]
        elif isinstance(x, tuple):
            return tuple(move_to_device(vv) for vv in x)
        else:
            return x

    if "dataset_name" in targets[0]:
        # for ["refcoco", "refcoco+", "refcocog"] evaluation
        return [{k: move_to_device(v) for k, v in t.items() if k not in ["caption", "dataset_name", "original_id"]} for t in targets]
    else:
        return [{k: move_to_device(v) for k, v in t.items() if k not in ["caption", "dataset_name", "original_id", "image_id"]} for t in targets]
