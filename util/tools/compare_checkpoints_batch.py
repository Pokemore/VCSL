...
def rescale_bboxes(boxes, size):
    # boxes in normalized cxcywh -> rescale to size (w,h)
    w, h = size
    boxes_xyxy = []
    for b in boxes:
        cx, cy, bw, bh = b
...
def draw_boxes_on_image(pil_img, boxes_xyxy, color, label=None, width=3):
...
def run_batch(ckpt1, ckpt2, image_paths, out_dir, device='cuda:0'):
    ...
    # build models (two separate model instances so they don't share buffers unexpectedly)
    ...
    for img_path in image_paths:
        ...
        logits1 = out1['pred_logits'][0].detach().cpu()
        boxes1 = out1['pred_boxes'][0].detach().cpu()
        logits2 = out2['pred_logits'][0].detach().cpu()
        boxes2 = out2['pred_boxes'][0].detach().cpu()

        # pick max over classes per query then pick top query (like inference_rsvg)
        scores1 = logits1.sigmoid().squeeze(0).max(-1)[0]
        best_idx1 = int(scores1.argmax().item())
        box1 = boxes1[0, best_idx1].numpy() if boxes1.ndim == 3 else boxes1[best_idx1].numpy()

        scores2 = logits2.sigmoid().squeeze(0).max(-1)[0]
        best_idx2 = int(scores2.argmax().item())
        ...
        # visualization: overlay both boxes on original image copy
        ...
        out_img_path = out_dir / (Path(img_path).stem + '_compare.png')
        ...
        entry = {
            ...
            'ckpt1_logits_mean': float(logits1.mean().item()),
            'ckpt1_logits_std': float(logits1.std().item()),
            'ckpt2_logits_mean': float(logits2.mean().item()),
            'ckpt2_logits_std': float(logits2.std().item()),
            'ckpt1_box': box1.tolist(),
            'ckpt2_box': box2.tolist(),
        }
        stats.append(entry)
...
if __name__ == '__main__':
    ...
    run_batch(args.ckpt1, args.ckpt2, imgs, args.out_dir, device=args.device)
