...
from init_sfd import initialize_sfd_module

import os
import numpy as np
import torch

# Apply the PyTorch compatibility patch
# import torch_patch
...
def main(args):
    ...
    args.masks = False
    assert args.dataset_file in ["rsvg", "rsvg_mm", "refcoco", "refcoco+", "refcocog","rsvg_opt", "all"]
    ...
    if getattr(args, "use_sfd", False): #if you open sfd module
        print("Initializing SFD module...")
        initialize_sfd_module(model_without_ddp, data_loader_train, device, num_batches=100)
    ...
    if args.resume:
        ...
        if not args.eval and 'optimizer' in checkpoint and 'lr_scheduler' in checkpoint and 'epoch' in checkpoint:
            import copy
            # Always override checkpoint optimizer/scheduler learning-related fields with current args
            p_groups = copy.deepcopy(optimizer.param_groups)
            optimizer.load_state_dict(checkpoint['optimizer'])
            # Replace lr/initial_lr/weight_decay in loaded optimizer with current param group values
            print('Loaded optimizer from checkpoint; overwriting lr/initial_lr/weight_decay with current args:')
            ...
            print(f"Fast-forwarding lr_scheduler to epoch {args.start_epoch}...")
